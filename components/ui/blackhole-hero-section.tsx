...
export interface BlackHoleHeroSectionProps
   ...
   * is dimmed. Physically it is g cubed, and this dials the exponent. The film
 ...
 * path obeys u'' + u = 3M u squared with u = 1/r, and in Cartesian form that is
 *
 *     a = -3/2 h squared r / |r| to the fifth ,   h = |r cross v| , constant
 *
 * with the horizon at |r| = 1. Leapfrog that with a step short enough to
...
void gasAt(vec3 p, float rd, float dt, out float dens, out vec3 tint, out float heat) {
  ...
  // Kepler: omega goes as r to the -3/2, so every radius turns at its own rate
  // and the clouds are read in a frame that turns with them. That shear is
  // what draws the trailing spirals, nothing here draws a spiral.
  ...
  // Shakura-Sunyaev: T falls as r to the -3/4. The colour ramp rides it.
...
void main() {
  ...
  float h2 = dot(hv, hv);
  float h = sqrt(h2);
  /** Angle swept around the hole so far. dphi/ds = h/r squared, and h is constant. */
  float swept = 0.0;
  ...
  for (int i = 0; i < MAX_STEPS; i++) {
    ...
    // faint: every extra turn costs a factor of about e to the 2pi. Given full
    ...
    if (rd > uDiskIn && rd < uDiskOut && abs(mid.y) < uThick * 5.0) {
      ...
      if (dens > 0.001) {
        ...
        // as g cubed, and uDoppler dials that exponent down to nothing.
    ...
    // u'' + u = 3M u squared, in Cartesian form.
...
export function BlackHoleHeroSection({
  ...
  useEffect(() => {
    ...
    function render(t: number) {
      ...
      const hj = HALTON[settled % HALTON.length];
